...
module.exports = (req, res, next) => {
  ...
  redis_util.getModule(req, key, req.params.componet, req.params.module, (err, _id, fields) => {
    ...
    req.body[req.params.field] = [req.body[req.params.field]];
    ...
    push[req.params.field] = vput.body[req.params.field][0];

    //console.log('VPUT', JSON.stringify(push));
    

    function savePut() {
      let name_collection = `${_id}_${req.params.componet}_${req.params.module}`;
      let collection = mongoose.db.collection(name_collection.toLowerCase());
      mongo_util.pushById(collection, req.params.id, push, req.query.return, (err, item) => {
        if (err) {
          //console.log(err);
          if (err.code === 11000) {
